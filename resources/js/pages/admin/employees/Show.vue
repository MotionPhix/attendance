...
import { ArrowLeft, Calendar, Check, Clock, DollarSign, Edit2, FileText, UserX, X } from 'lucide-vue-next';
...
const loadAttendancePage = (page: number) => {
  router.get(route('admin.employees.show', props.employee.id), { attendance_page: page }, { preserveState: true });
};

const loadLeavePage = (page: number) => {
  router.get(route('admin.employees.show', props.employee.id), { leave_page: page }, { preserveState: true });
};

const loadSalaryPage = (page: number) => {
  router.get(route('admin.employees.show', props.employee.id), { salary_page: page }, { preserveState: true });
};
...
function editEmployeeModal() {
  visitModal(route('admin.employees.edit', props.employee.id))
}
    ...
    <div class="container py-6">
      ...
      <div class="mb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-muted-foreground">{{ employee.position }} at {{ employee.department.name }}</p>
        </div>
        ...
        <div class="mb-6 flex flex-1 items-center justify-end">
          <div class="flex items-center gap-2">
            <Button
              variant="outline"
              @click="editEmployeeModal">
              <Edit2 class="mr-2 h-4 w-4" />
              Edit Details
            </Button>

            <Button v-if="employee.status !== 'terminated'" variant="destructive" @click="showDeactivateDialog = true">
              ...
              <CardContent>
                <dl class="grid gap-y-4 divide-y">
                  <div class="grid gap-y-2">
                    ...
                    <dd>
                      <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium" :class="getStatusColor(employee.status)">
        ...
        <TabsContent value="attendance">
          <Card>
            <CardHeader>
              ...
              <CardDescription> View attendance records and patterns for this employee. </CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <div class="rounded-md border">
                <table class="w-full">
                  ...
                  <tbody>
                    <tr v-for="record in attendanceRecords.data" :key="record.id" class="border-b">
                      ...
                      <td class="p-3">
                        <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium" :class="getStatusColor(record.status)">
              ...
              <div v-if="attendanceRecords.last_page > 1" class="mt-4 flex items-center justify-between">
                <p class="text-sm text-muted-foreground">Showing {{ attendanceRecords.per_page }} of {{ attendanceRecords.total }} records</p>
        ...
        <TabsContent value="leave">
          <Card>
            <CardHeader>
              ...
              <CardDescription> View leave requests and their status. </CardDescription>
            </CardHeader>
            <CardContent>
              <div class="rounded-md border">
                <table class="w-full">
                  ...
                  <tbody>
                    <tr v-for="request in leaveRequests.data" :key="request.id" class="border-b">
                      <td class="p-3">{{ request.type }}</td>
                      <td class="p-3">{{ formatDate(request.start_date) }}</td>
                      <td class="p-3">{{ formatDate(request.end_date) }}</td>
                      <td class="p-3">
                        <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium" :class="getStatusColor(request.status)">
                          {{ request.status }}
                        </span>
                      </td>
              ...
              <!-- Leave Pagination -->
              <div v-if="leaveRequests.last_page > 1" class="mt-4 flex items-center justify-between">
                <p class="text-sm text-muted-foreground">Showing {{ leaveRequests.per_page }} of {{ leaveRequests.total }} requests</p>
        ...
        <TabsContent value="salary">
          <Card>
            <CardHeader>
              ...
              <CardDescription> View salary records and payment history. </CardDescription>
            </CardHeader>
            <CardContent>
              <div class="rounded-md border">
                <table class="w-full">
                  ...
                  <tbody>
                    <tr v-for="record in salaryRecords.data" :key="record.id" class="border-b">
                      ...
                      <td class="p-3 text-green-600">
                        ...
                        <span class="text-xs text-muted-foreground"> ({{ record.overtime_hours }}h) </span>
                      </td>
                      <td class="p-3 text-red-600">-{{ formatCurrency(record.deductions) }}</td>
              ...
              <!-- Salary Pagination -->
              <div v-if="salaryRecords.last_page > 1" class="mt-4 flex items-center justify-between">
                <p class="text-sm text-muted-foreground">Showing {{ salaryRecords.per_page }} of {{ salaryRecords.total }} records</p>
      ...
      <Dialog v-model:open="showDeactivateDialog">
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription> Are you sure you want to terminate this employee? This action cannot be undone. </DialogDescription>
          </DialogHeader>

          <DialogFooter>
            <Button variant="outline" @click="showDeactivateDialog = false"> Cancel </Button>
            <Button variant="destructive" :disabled="processing" @click="handleDeactivateEmployee"> Terminate </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
